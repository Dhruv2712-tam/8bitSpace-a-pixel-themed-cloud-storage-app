...
export async function loadCloud(user) {
  ...
  return {
    ...
    profile: {
      ...
      avatar: profileRow.avatar_url || '/avatars/avatar-01.jpeg',
...
export async function uploadCloudFiles(user, folderId, files) {
...
export async function saveCloudProfile(user, profile) {
  const row = must(await supabase.from('profiles').upsert({
    id: user.id, display_name: profile.name.trim(), avatar_url: profile.avatar,
  ...
  return { ...profile, name: row.display_name, email: user.email || '', avatar: row.avatar_url }
}

export async function setCloudTrash(user, item, trashed) {
  const value = trashed ? new Date().toISOString() : null
  if (item.type === 'folder') {
    must(await supabase.from('files').update({ trashed_at: value }).eq('folder_id', item.id))
    must(await supabase.from('folders').update({ trashed_at: value }).eq('id', item.id))
  } else {
